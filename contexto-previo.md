...
- **Fuente de datos**: `get_nominas_listado(ejercicio, mes, empresa_id, incluir_anuladas)` devuelve **los dos ámbitos** de líneas (`persona`, que suma, y `puesto`, el desglose por contrato con su `historial_id` y `puesto` resueltos) con la cabecera denormalizada, para armar la matriz en cliente **sin toparse con el tope de 1000 filas de PostgREST** (una nómina truncada sería un error grave). `buildGestionNominaListado` los separa: mezclarlos duplicaría cada importe. Filtra por las columnas generadas `ejercicio`/`mes`.
- **Todo el detalle en los tres formatos**: salen todos los conceptos, sin filtrar. Las **subfilas del prorrateo** (`detalle_de`) no son columna de la matriz (su padre ya lleva la suma y se contaría dos veces), pero aparecen anidadas en el panel, indentadas en el PDF y en la hoja Detalle del Excel con columnas propias «Importe subfila» y «Subfila de». El **desglose por puesto** va en la hoja «Por puesto» del Excel y en el PDF, y solo se imprime cuando la nómina junta dos contratos.
