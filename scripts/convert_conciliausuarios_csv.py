...
def build_insert(rows):
    lines = [
        "insert into public.conciliausuarios (",
        "  " + ", ".join(COLUMNS),
        ") values",
    ]
...
def write_chunked_inserts(rows, output_dir, chunk_size):
    ...
    for index, start in enumerate(range(0, len(rows), chunk_size), start=1):
        ...
        chunk_path = output_dir / f"seed_conciliausuarios_{index:03}.sql"
    ...
    manifest.write_text(
        "\n".join(
            [
                ...
                "",
                "Ejecuta estos archivos en el SQL Editor de Supabase despues de `supabase/tables/conciliausuarios.sql`.",
                "",
...
def main():
    if len(sys.argv) not in (3, 4, 5):
        print(
            "Usage: convert_conciliausuarios_csv.py input.csv output [--csv | --chunk-size rows]",
            file=sys.stderr,
        )
        return 2
