...
function renderCandidates(rows) {
  if (!rows.length) {
    tableBody.innerHTML = `
      <tr>
        <td colspan="11" class="empty-state">No hay datos cargados todavia.</td>
      </tr>
    `;
    return;
  }
...
async function init() {
  ...
  setStatus(
    currentSession
      ? "Conectado a Supabase. El panel publico inserta datos en la base y el privado lee desde Supabase."
      : "Supabase configurado. El panel publico ya envia datos a Supabase; el panel privado necesita login.",
    "success"
  );
}
