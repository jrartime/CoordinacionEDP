...
select
  ...
  (least(coalesce(pr.a_baja,'infinity'::date), coalesce(pr.b_baja,'infinity'::date)) - pr.b_alta + 1) as dias_solape,
...
comment on view public.historiales_laborales_solapes is
  'Pares de periodos solapados de la misma persona y puesto. Cada par duplica devengos en calcular_nomina_persona. Filtrar por legitimo_baja_variacion = false para ver solo las anomalias reales.';
