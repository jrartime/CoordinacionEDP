...
-- Identity para poder añadir tipos nuevos desde la UI (panel "Tipos de
-- permiso" en Permisos) sin tener que calcular el siguiente id a mano. Los
-- ids 1-10 sembrados arriba se conservan; el arranque se reposiciona por
-- encima del máximo actual cada vez que se corre este fichero.
do $$
begin
  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'personal_permisos_tipo'
...
select setval(
  pg_get_serial_sequence('public.personal_permisos_tipo', 'id'),
  coalesce((select max(id) from public.personal_permisos_tipo), 1),
  true
);
...
declare v_seq text;
begin
  v_seq := pg_get_serial_sequence('public.personal_permisos_tipo', 'id');
