...
- **Gestión** — Pestaña transversal que cruza historiales laborales y registros por intervalo de fechas. Filtros de fecha (Desde/Hasta), empresa y personal (solo personas con registros en el intervalo); muestra historiales solapados y un pivote de horas. Con una persona seleccionada permite abrir/guardar su ficha de Personal, abrir cada Historial pulsando su nombre y desplegar los registros reales del periodo pulsando el pivote de horas. Incluye `Opciones nómina` en una columna extensible. Agregados server-side (`get_gestion_registros_resumen`, `get_gestion_personal`).
...
- **No cuentan como solape** las filas sin horas (`horas` a 0/null): son CAMB (lo cubrió otro) o LG (licencia), que esa persona no trabaja y por tanto no ocupan su horario. Mismo criterio que `withRecordSituacionSideEffects`.
...
- **Nóminas emitidas** (`supabase/tables/nominas.sql`): una nómina emitida no es solo el recibo, es el **registro laboral** del periodo. Cuatro tablas, todas admin-only:
  ...
  - **Listado mensual (Fase 7)**: bloque **Listado de nóminas del mes** en Gestión, transversal (no depende de la persona filtrada, solo admin). Filtros mes/año/empresa/incluir-anuladas. Muestra una **matriz persona × concepto** (una columna por concepto, ordenadas por `orden`; primera columna fija; totales al pie) y, debajo, el **detalle apilado** de cada recibo. Exporta a **Excel** (hoja Resumen = matriz + hoja Detalle = formato largo) y a **PDF** (resumen de totales + detalle por nómina). Fuente de datos: `get_nominas_listado(ejercicio, mes, empresa_id, incluir_anuladas)` — devuelve las líneas de ámbito persona con la cabecera denormalizada, para armar la matriz en cliente **sin toparse con el tope de 1000 filas de PostgREST** (una nómina truncada sería un error grave). Filtra por las columnas generadas `ejercicio`/`mes`.
